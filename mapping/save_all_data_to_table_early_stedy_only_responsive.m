...
% A map showing the (14) cells that are transient light-responsive, (15) cells that are sustained 
% light-responsive, (13) cells that are both transient and sustained  light-responsive, and (16) non responding cells.
% 
clear
...
number_of_letters_in_data_file_name = 17;
...
for i = 1:length(datafile)
    ...
    for j = 1:length(cells)
        ...
        exp_name{count, 1} = file{i}(1:number_of_letters_in_data_file_name);
        ...
        if sum(current_cell.Is_reponsive(1:2)) == 2
            cluster = [cluster; 13];
        elseif current_cell.Is_reponsive(1)
            cluster = [cluster; 14];
        elseif current_cell.Is_reponsive(2)
            cluster = [cluster; 15];
        else
            cluster = [cluster; 16];
        end
...
writetable(brain_render_table, 'brain_Render_early_stedy_only_responsive.csv', 'Delimiter', ',')
