...
function wf = getbestWaveForms(gwfparams)
%
...
for curUnitInd=1:numUnits
    ...
    for curSpikeTime = 1:min([gwfparams.nWf curUnitnSpikes])
        if spikeTimeKeeps(curUnitInd,curSpikeTime)+gwfparams.wfWin(end) > mmf_length
            continue
        end
        tmpWf = mmf.Data.x(gwfparams.best_channels(curUnitInd),spikeTimeKeeps(curUnitInd,curSpikeTime)+gwfparams.wfWin(1):spikeTimeKeeps(curUnitInd,curSpikeTime)+gwfparams.wfWin(end));
...
wf.unitIDs = unitIDs;
wf.channels = gwfparams.best_channels;
