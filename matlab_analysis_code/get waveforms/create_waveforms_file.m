...
%% run teh function and get all the wave forms
...
%% extract and plot waveform from the best channel for all cells
for i = 1:length(wf.unitIDs)
    best_channel = w.ch(i) + 1;
    best_waveform.waveform(i,:,:) = wf.waveForms(i,:, best_channel, :);
    best_waveform.mean(i, :) = wf.waveFormsMean(i, best_channel, :);
...
%%
save('best_waveform.mat', 'best_waveform')
...
%% extract and plot waveform from the best channel of 5 cells
w = tdfread('cluster_info.tsv');
for i = 1:5
    best_channel = w.ch(i) + 1;
    all_wave_form(i,:,:) = wf.waveForms(i,:, best_channel, :);
    wave_form_matrix = (squeeze(all_wave_form(i,:,:)))';
    figure
    subplot(1,2,1)
    plot(wave_form_matrix)
    subplot(1,2,2)
    plot(squeeze(wf.waveFormsMean(i,best_channel,: )))
