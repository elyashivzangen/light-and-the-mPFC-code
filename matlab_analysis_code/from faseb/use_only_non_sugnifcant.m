...
mkdir(['deleted_cells_' num2str(numofint)])
        ...
        for w = 1:numofint
            ...
            if current_cell(w2).on.value > normpval && current_cell(w2).off.value >  normpval && current_cell(w2).sustained.value >  normpval %no sugnificant response
                if w == numofint %cheak if it is the last intensity to use
                    all_data_deleted.(cells{j}) = new_all_data.(cells{j}); %save non sugnificant cells
...
%     experiment_name = [experiment_name repmat(file{i}, j, 1)];
    ...
    deleted_cells_path = [path 'deleted_cells_' num2str(numofint)];
    datafile_deleted = fullfile(deleted_cells_path, file{i}); %save path
    all_data = all_data_deleted;
    save(datafile_deleted, 'all_data')
