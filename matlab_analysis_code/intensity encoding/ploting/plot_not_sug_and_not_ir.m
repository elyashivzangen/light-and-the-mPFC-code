...
for i = 1:length(datafile)
    ...
    for j = 1:length(cells)
        ...
        if ~current_cell.new_fit3(w).isIntEncoding && ~current_cell.Is_reponsive(w)
            ...
            title(['pvalue = ',num2str(current_cell.pval_table(2, 3)) ' baseline = ' num2str(mean(current_cell.baseline_vector.mean))]);
            ...
            legend off
            title(['rmse = ',num2str(fd.original_rmse),'   n = ',num2str(fd.original_curve.n), 'P10 = ',num2str(fd.P10_mean), 'exp = ' file{i}, cells{j}]);
            exportgraphics(f1,'not_responsive_not_ir2.pdf','Append',true)
            close all
        end
    end
end
